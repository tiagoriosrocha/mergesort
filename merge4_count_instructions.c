...
// Função Bubble Sort com contagem de comparações
void bubbleSort(int array[], int left, int right)
{
    for (int i = left; i <= right; i++)
    {
        ...
        {
            comparisons++; // conta a comparação array[j] > array[j+1]
...
void merge(int *array, int *temp, int left, int mid, int right)
{
    ...
    while (i <= mid && j <= right)
    {
        comparisons++; // conta a comparação temp[i] <= temp[j]
        if (temp[i] <= temp[j])
            array[k++] = temp[i++];
        else
            array[k++] = temp[j++];
...
int main()
{
    ...
    for (int s = 0; s < NUM_SIZES; s++)
    {
        ...
        {
            long long comps = test_sort(original, n, thresholds[t]);
            printf("\t%lld", comps);
        }
