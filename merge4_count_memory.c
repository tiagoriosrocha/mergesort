...
// Merge Sort híbrido com bubble para sub-vetores pequenos
void hybridSort(int *array, int *temp, int left, int right, int threshold)
{
...
size_t test_sort(int *original, int n, int threshold)
{
    size_t memory_allocated = 0;

    int *array = malloc(n * sizeof(int));
    memory_allocated += n * sizeof(int);
    ...
    memory_allocated += n * sizeof(int); // conta cópia, opcional
...
int main()
{
    ...
    for (int s = 0; s < NUM_SIZES; s++)
    {
        ...
        size_t memory_allocated_total = 0;

        // Gera vetor original uma vez
        int *original = malloc(n * sizeof(int));
        memory_allocated_total += n * sizeof(int);
        if (!original)
        {
        ...
        {
            size_t mem = test_sort(original, n, thresholds[t]);
            memory_allocated_total += mem;
            printf("\t%zu", mem);
        }
